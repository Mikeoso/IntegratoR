...
### Security
- **Key values containing control characters are rejected.** The atomic `$batch` body is assembled as text, so a composite-key or scalar key value carrying CR or LF ended up in the embedded `METHOD url HTTP/1.1` request line and could terminate it early, injecting headers — or a forged request — into that part. The other write paths were never exposed: they hand the URL to `System.Uri`, which rejects a stray CR or LF, and only the text-assembled atomic path lacked that safety net. Entity payloads were never affected either, because `System.Text.Json` escapes control characters and batch boundaries are per-request GUIDs. Found by review of the same defect on the `support/1.3` hotfix line.
