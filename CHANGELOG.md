...
### Security
- **Key values and entity-set names containing control characters are rejected.** The atomic `$batch` body is assembled as text, so a value carrying CR or LF ended up in the embedded `METHOD url HTTP/1.1` request line and could terminate it early, injecting headers — or a forged request — into that part. This reached the wire through composite-key values, scalar key values, and the `entitySet` parameter, which for a batch create is the entire relative URL. The other write paths were never exposed: they build an `HttpRequestMessage`, and `HttpClient` serialises the request line from the canonical `Uri` form, which percent-encodes CR/LF to `%0D%0A` — escaping, not rejection, so anything that reads `Uri.OriginalString` instead would lose that cover. Entity payloads were never affected either, because `System.Text.Json` escapes control characters and batch boundaries are per-request GUIDs. Found by review of the same defect on the `support/1.3` hotfix line.

### Fixed
- A failed batch no longer reports a success status code. When an atomic changeset did not commit but no sub-response explained why — none parsed, or all of them 2xx yet not reconcilable with the operations sent — the per-operation result borrowed a 2xx from the response. Such a batch now reports `502`, because the failure is that the answer could not be used, not that a particular operation was rejected.
